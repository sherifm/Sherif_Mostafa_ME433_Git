...
//Helper functions for car robot
int get_ocrs(int);
void ControlUpdate(int);
//The timers, OCRs etc are set-up in sys_ports_static.c
...
//            THIS BLOCK OF CODE READS FROM UART!!!!!
...
//            if(appData.readBuffer[0]!='0') BSP_LEDOn(APP_USB_LED_2);
//            else BSP_LEDOff(APP_USB_LED_2);
//            rxbuffer_index++;
...
//            THIS BLOCK OF CODE READS FROM ANDROID

            int COM;


            strcpy(rxbuffer,appData.readBuffer);
            display_text(rxbuffer,5);//Print COM to screen for debugging

            sscanf('%d %d',&COM,&num1);//Num 1 is used for debuggning with LED

            ControlUpdate(COM);
//            if(appData.readBuffer[i]=='a') BSP_LEDOn(APP_USB_LED_2);
