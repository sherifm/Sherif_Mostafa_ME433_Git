...
int main(int argc, char* argv[])
{
	...
	short MAF_accels[NUM_DATAPOINTS][3]; //List of datasets with x,y,z accelerations
	...
	while(j<=NUM_DATAPOINTS){
		...
		// while(buf[0]!=1){
		// 	if(buf[0]==0){printf("Empty packet received")}
		// buf[0] = 0x00; 	//The first byte is the report number (0x0).
		// buf[1] = 0x02; //Instruct the PIC to reply with data
		// res = hid_write(handle, buf, 65);
		// // Read requested stat
		...
		if(buf[0]==1){
			...
			//Unfiltered data
			lsb = buf[5];//Read least significant byte of an axis-accel
			msb = buf[6];//Read most significant  byte of an axis-accel
			accels[j][2] = (msb << 8 ) | (lsb & 0xff); //Combine lsb and msb

			//Filtered data
			lsb = buf[7];//Read least significant byte of an axis-accel
			msb = buf[8];//Read most significant  byte of an axis-accel
			MAF_accels[j][2] = (msb << 8 ) | (lsb & 0xff); //Combine lsb and msb
			j++;
		}
		// else{printf("Empty packet received\n");}
	}
	...
	//Print all 3 accelerations to a file
	// for(j=0;j<=NUM_DATAPOINTS; j++){
	// 	fprintf(fp,"%3.2f %3.2f %3.2f\n",((float)accels[j][0])/16000,((float)accels[j][1])/16000,((float)accels[j][2])/16000);
	// }

	//Print z data only to a file
	for(j=0;j<=NUM_DATAPOINTS; j++){
		fprintf(fp,"%3.2f %3.2f\n" ,((float)accels[j][2]),((float)MAF_accels[j][2]));
		//Divide accels by 1600 to have expression in factors of g
	}
