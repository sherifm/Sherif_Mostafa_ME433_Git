    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_MAIN_TASK:
            ...
            else if( appData.hidDataReceived )
            {
                ...
                {
                    case 0x01://First bit of buffer ='1' signals the PIC to read text message and display
                    {
                        char usb_text[25];//String to hold incoming text
                        int k;//Counter
                        int text_start; //Indicates where the text starts in the buffer
                        int row; //Indicates the row to display the text

                        /* Toggle on board LED1 to LED2. */
                        BSP_LEDToggle( APP_USB_LED_1 );
                        BSP_LEDToggle( APP_USB_LED_2 );

                        //Indexing of trnasmitDataBuffer and hid.cpp buf[] shifted!!!
                        row = (int)appData.receiveDataBuffer[1];//Row number
                        text_start=2;//Beginning of text in buffer at pos 2
                        k = 0; //Counter variable
                        //This only reads string without spaces
                        while((char)appData.receiveDataBuffer[text_start+k]!= '\0'){//Read buffer until reaching termination character
                            usb_text[k] = (char)appData.receiveDataBuffer[text_start+k];//Read text from buffer into string
                            k++;
                        }
//
                        display_text(usb_text,row);//Display text on OLED

//                        sprintf(usb_text,"Hello!"); //Message for debugging
//                        display_text(usb_text,row);//Display text on OLED
                    ...
                    case 0x02://First bit of buffer ='2' signals the PIC to send back accelerometer data
                    {
                        short accels[3]; // accelerations for the 3 axes
                        ...
                        if(appData.hidDataTransmitted)
                        {
                            ...
                            //Indexing of trnasmitDataBuffer[] and hid.cpp buf[] align
                            //xData
                            appData.transmitDataBuffer[1] = accels[0] & 0xFF; //LSB
                            appData.transmitDataBuffer[2] = accels [0] >> 8;  //MSB
                            //yData
                            appData.transmitDataBuffer[3] = accels[1] & 0xFF; //LSB
                            appData.transmitDataBuffer[4] = accels [1] >> 8;  //MSB
                            //zData
                            appData.transmitDataBuffer[5] = accels[2] & 0xFF; //LSB
                            appData.transmitDataBuffer[6] = accels [2] >> 8;  //MSB

//                            display_vals(accels);//For debugging
//                            to varify that hos is printing correct accels
                            
                            if(_CP0_GET_COUNT()>200000){
//                                _CP0_SET_COUNT(0);
                                appData.transmitDataBuffer[0] = 0x01;//A 1 in buf[0] tells
                                //the host to read the packet, 0-> ignore
                            }
                            else{
                                appData.transmitDataBuffer[0] = 0x00;//A '0' in buf[0] indicates an
                                //empty packet that the host should ignore
                            }

//                            Previously hid_basic returned the button state
//                            if( BSP_SwitchStateGet(APP_USB_SWITCH_1) == BSP_SWITCH_STATE_PRESSED )
//                            {appData.transmitDataBuffer[1] = 0x00;}
//                            else{appData.transmitDataBuffer[1] = 0x01;}
